...
# lsst.org/qserv-operator-bundle:$VERSION and lsst.org/qserv-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= qserv/qserv-operator
...
yaml: manifests kustomize ## Generate operator.yaml
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > $(ROOT_DIR)/manifests/operator.yaml

yaml-ns-scoped: ## Generate operator-ns-scope.yaml
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
...
kustomize: ## Download kustomize locally if necessary.
	$(call go-get-tool,$(KUSTOMIZE),sigs.k8s.io/kustomize/kustomize/v4@v4)
